...
bool MainPage_CallBack(const Page *cur_page_addr, InputMsg msg)
{
    ...
    if (msg_click == msg)
    { // 可以依靠text识别选项
        if (!strcmp(select_item->text, "! WouoUI"))
        { // 设置消息弹窗内容并跳转
        ...
        else if (!strcmp(select_item->text, "% Spin"))
        { // 设置spin弹窗的设置范围和小数点数并跳转
...
bool SettingPage_CallBack(const Page *cur_page_addr, InputMsg msg)
{
    Option *select_item = WouoUI_ListTitlePageGetSelectOpt(cur_page_addr);
    if (msg_click == msg)
    { // 也可以靠选项的order顺序来识别item(以下顺序为其在数组中的下标)
...
bool BgBlurSelPage_CallBack(const Page *cur_page_addr, InputMsg msg)
{
    ...
    {
    case msg_click:
        if (bg_blur_sel_page.sel_str_index < 5)
        { // click时选中的选项有效
        ...
        else if (bg_blur_sel_page.sel_str_index == 5)
        { // 测试跳转嵌套弹窗
